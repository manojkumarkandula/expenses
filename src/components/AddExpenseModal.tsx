...
import { X, Utensils, Car, ShoppingBag, HeartPulse, BookOpen, Film, Receipt, CircleEllipsis } from 'lucide-react';
import { Transaction } from '../types';

interface AddExpenseModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: { amount: number; category: string; date: string; notes: string; title: string }) => void;
  currency: string;
...
export default function AddExpenseModal({ isOpen, onClose, onSave, currency, editTransaction }: AddExpenseModalProps) {
    ...
    >
      ...
      >
        <div className="flex justify-between items-center mb-6">
          <h2 id="add-expense-title" className="text-xl font-bold text-neutral-900 dark:text-neutral-50 tracking-tight">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          {/* Amount input */}
          <div>
            <label htmlFor="expense-amount" className="block text-xs font-semibold text-neutral-600 dark:text-neutral-400 uppercase tracking-wider mb-2 font-mono">
              Amount ({currency}) *
            </label>
          ...
          {/* Large Category Cards Grid - No Dropdowns */}
          <div>
            ...
            <div className="grid grid-cols-2 xs:grid-cols-4 gap-2" role="radiogroup" aria-label="Expenses category list">
              {CATEGORIES.map((item) => {
                ...
                return (
                  ...
                  >
                    <Icon className="w-5 h-5 mb-1.5 flex-shrink-0 text-neutral-700 dark:text-neutral-300 group-hover:text-neutral-900" />
          ...
          {/* Title input */}
          <div>
            <label htmlFor="expense-title" className="block text-xs font-semibold text-neutral-600 dark:text-neutral-400 uppercase tracking-wider mb-2 font-mono">
              Spent On / Title
            </label>
            <input
              type="text"
              id="expense-title"
              placeholder="e.g. Lunch with team, Bus ticket, Grocery store"
              value={title}
          ...
          {/* Date input */}
          <div>
            <label htmlFor="expense-date" className="block text-xs font-semibold text-neutral-600 dark:text-neutral-400 uppercase tracking-wider mb-2 font-mono">
              Date *
            </label>
            <input
              type="date"
          ...
          {/* Notes input */}
          <div>
            <label htmlFor="expense-notes" className="block text-xs font-semibold text-neutral-600 dark:text-neutral-400 uppercase tracking-wider mb-2 font-mono">
              Notes (Optional)
            </label>
            <textarea
              id="expense-notes"
          ...
          <div className="pt-2 flex gap-3">
            ...
            >
              {editTransaction ? 'Save Changes' : 'Record Expense'}
            </button>
          </div>
        </form>
      </div>
